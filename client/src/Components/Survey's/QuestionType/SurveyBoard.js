...
class SurveyBoard extends Component {
  constructor() {
    super();
    this.state = {
      picked_answers: {},
  ...
  getSurvey = (user_id) => {
    axios.get("/getSurvey", (req, res) => {
      if (typeof res === "object") {
        this.setState({ surveyArray: res, totalQuestions: res.length });
        this.props.openModal();
      } else {
        console.log("err");
        this.props.closeModal();
  ...
  handleAnswer = (picked_answers) => {
    ...
    //If user's answers
    if (
      Object.keys(picked_answers).length === this.state.totalQuestions &&
      Object.keys(picked_answers).filter(
        (key) => picked_answers[key].length > 0
